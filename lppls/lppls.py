...
class LPPLS(object):
    ...
    def plot_fit(self, show_tc=False):
        ...
        ax1.plot(t_obs, price, label='price', color='black', linewidth=0.75)
        ax1.plot(t_obs, lppls_fit, label='lppls fit', color='blue', alpha=0.5)
        # if show_tc:
    ...
    def compute_indicators(self, res):
        pos_lst = []
        neg_lst = []
        pos_conf_lst = []
        neg_conf_lst = []
    ...
    def plot_confidence_indicators(self, res):
        ...
        ax2_0.plot(ts, res_df['neg_conf'], label='bubble indicator (neg)', color='blue', alpha=0.5)
    ...
    def mp_compute_nested_fits(self, workers, window_size=80, smallest_window_size=20, outer_increment=5, inner_increment=2, max_searches=25, filter_conditions_config={}):
        ...
        func_arg_map = [(
            ...
            inner_increment,
            max_searches,
        ) for i in range(0, obs_opy_len, outer_increment)]
    ...
    def _func_compute_nested_fits(self, args):
        ...
        for j in range(0, n_fits, inner_increment):
            ...
            tc, m, w, a, b, c, c1, c2, O, D = self.fit(max_searches, minimizer='Nelder-Mead', obs=obs_shrinking_slice)
